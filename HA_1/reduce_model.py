...
def process_image(x):
    ...
    # Tensor('batch:1', shape=(1, 512, 512, 3), dtype=float32) -> 1 batch size
...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    layer_specs = [
        ngf * 2,  # encoder_2: [batch, 256, 256, ngf * 2] => [batch, 128, 128, ngf * 4]
        ngf * 4,  # encoder_3: [batch, 128, 128, ngf * 4] => [batch, 64, 64, ngf * 8]
        ngf * 8,  # encoder_4: [batch, 64, 64, ngf * 4] => [batch, 32, 32, ngf * 8]
        ngf * 8,  # encoder_5: [batch, 32, 32, ngf * 4] => [batch, 16, 16, ngf * 8]
        ngf * 16,  # encoder_6: [batch, 16, 16, ngf * 8] => [batch, 8, 8, ngf * 8]
        ngf * 16,  # encoder_7: [batch, 8, 8, ngf * 8] => [batch, 4, 4, ngf * 8]
        ngf * 16,  # encoder_8: [batch, 4, 4, ngf * 8] => [batch, 2, 2, ngf * 8]
        ngf * 16,  # encoder_9: [batch, 2, 2, ngf * 8] => [batch, 1, 1, ngf * 8]
    ]
...
if __name__ == '__main__':
    ...
    x = tf.placeholder(tf.uint8, shape=(512, 1024, 3), name='image_tensor')  # input tensor
    y = generate_output(x)
